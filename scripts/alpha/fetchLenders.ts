...
const getPastLenders = async (
  ...
  if (lToken && lTokenInterface) {
    ...
    const promises = uniqueAddresses.map((address) =>lToken.getAccountSnapshot(address))
    ...
    const lenderResult = results.map((res, index) => {
      const [err, balance, borrowBal, exchangeRate] = res;

      return {
        balance: Number(formatEther(balance)) * Number(formatEther(exchangeRate)),
