...
async function main() {
  ...
  const kkubLending = KUBLending__factory.connect(addressList['KKUBLending'], owner);
  ...
  const lTokenAddr = await kkubLending.lToken();
  const lToken = LToken__factory.connect(lTokenAddr, owner);

  console.log("KKUB accepted KYC: ", await lToken.acceptedKYCLevel());
